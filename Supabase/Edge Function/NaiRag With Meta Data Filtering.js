...
Deno.serve(async (req)=>{
  try {
    // Grab the user's query and optional parameters from the JSON payload
    const { query, filter = {}, match_count = 10, distance_method = 'cosine' } = await req.json();
    // Validate inputs
    if (!query || typeof query !== 'string') {
      throw new Error('Query is required and must be a string');
    }
    if (match_count <= 0) {
      throw new Error('match_count must be positive');
    }
    const validDistanceMethods = [
      'cosine',
      'euclidean',
    ...
    if (!validDistanceMethods.includes(distance_method)) {
      throw new Error(`Invalid distance method: ${distance_method}. Supported methods are: ${validDistanceMethods.join(', ')}`);
    }
    // Instantiate OpenAI client
    const openai = new OpenAI({
      apiKey: openaiApiKey
    ...
    // Instantiate the Supabase client
    const supabase = createClient(supabaseUrl, supabaseServiceRoleKey);
    // Call find_similar_vector_cosine Postgres function via RPC
    const { data: documents, error } = await supabase.rpc('search_similar_documents', {
      query_embedding: embedding,
      match_count: match_count,
